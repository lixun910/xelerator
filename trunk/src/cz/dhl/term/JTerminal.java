...
public class JTerminal extends JPanel implements ATerminal {
    ...
    final class FontInfo {
        ...
        void installFont(int style) {
            ...
            try {
                ...
                System.out.println(
                    "jTerm: Courier " + fName + size + " Not Available!");
            }
        }
    ...
    final class JView extends JPanel {
        ...
        public void paintComponent(Graphics g) {
            ...
            //System.out.println("paint clip:"+clip.fm.x+","+clip.fm.y+":"+clip.to.x+","+clip.to.y);
            //System.out.println("");

            //*** Empty Area *** 
            //FILLS ARE REQUIRED IF super.paintComponent(g); WASN'T CALLED
            //Dimension limits = view.getSize();
            //g.setColor(COLORS[DEFAULT.BACK]);
            //if(limits.width > toPxX(size.x))
            //    g.fillRect(toPxX(size.x),0, limits.width-toPxX(size.x),limits.height);
            //if(limits.height > toPxY(size.y))
            //    g.fillRect(0,toPxY(size.y), limits.width,limits.height-toPxY(size.y-1)); 
